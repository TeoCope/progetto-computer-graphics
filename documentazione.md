...
```
Il file `requirements.txt` contiene il freeze completo dell'ambiente Python, utile per ricreare l'ambiente da zero con `pip install -r requirements.txt` (istruzioni complete su installazione e download dei modelli body in `README.md`).
...
### Integrazione Unity
- La cartella `unity/` contiene ora un **progetto Unity completo e versionato** (Unity 6, apribile direttamente da Unity Hub senza setup manuale): include il pacchetto ufficiale **SMPL-X for Unity** di MPI (`Assets/SMPLX/`, licenza solo ricerca/didattica), il pacchetto **bmlSUP – SMPL Unity Player** (`Packages/com.biomotionlab.sup/`) con le animazioni mocap di esempio, e una copia di `presets.json` in `Assets/StreamingAssets/` (da riaggiornare dopo ogni `build_presets.py`). Gli script C# del progetto (`AvatarController`, `UIManager`, `AvatarPresets`, `FitServiceClient`, `SUPAnimationBridge`, più l'editor script `BuildAvatarUI` che genera e collega automaticamente la Canvas) replicano l'interfaccia dell'app Python — in italiano — con aggiornamento della forma **in tempo reale** tramite blendshapes. Poiché gli spazi di forma di SMPL e SMPL-X non sono compatibili, `build_presets.py` calcola per ogni preset anche la variante `betas_smplx` usata da Unity.
- Come nella GUI Python, **l'utente definisce l'avatar inserendo le misure in cm**: il bottone *Adatta alle Misure* invia le misure al server locale `avatar_server.py` (root del repo, avvio con `python avatar_server.py`), che riusa **esattamente** il misuratore geometrico `MeasureBody` e l'ottimizzatore L-BFGS-B condivisi in `fitting.py` e risponde con le betas e l'errore residuo. Lo stesso server calcola le *Current Measurements* reali del corpo corrente mostrate in Unity (endpoint `POST /measure`). Questa architettura garantisce che input antropometrico e avatar siano coerenti in entrambe le applicazioni, senza duplicare la logica in C#.
- La UI Unity offre inoltre:
  - **Texture**: un menu a tendina elenca le immagini presenti nella **stessa cartella `data/textures/` dell'app Python** e le applica a runtime al materiale dell'avatar (riapplicate anche al cambio genere).
  - **Espressioni facciali**: 10 slider che pilotano in tempo reale i blendshape di espressione FLAME di SMPL-X (le stesse Componenti Principali descritte nella sezione 5).
  - **Animazioni (SUP)**: riproduzione sull'avatar corrente delle animazioni mocap **AMASS** in formato JSON fornite da bmlSUP, tramite il nuovo script `SUPAnimationBridge` che ritargettizza i 52 joint SMPL-H sullo scheletro SMPL-X (con conversione del sistema di coordinate e correzione automatica di orientamento e altezza al primo frame); bottone *Riproduci/Ferma*, velocità e loop regolabili.
- **File > Save Model Params** salva, accanto al file joblib, anche un **JSON portabile** (`model_type`, `gender`, `betas`, misure correnti, eventuale `preset_id`), caricabile in Unity con "Load JSON". Setup della scena, dettagli della UI e delle animazioni in `unity/README.md`.
